...
void changeLevel(Level &lvl, int &numberLevel) {
	if (numberLevel == 1) { lvl.LoadFromFile("files/Level1.tmx"); }
	if (numberLevel == 2) { lvl.LoadFromFile("files/Level2.tmx"); }
}
void RunGame(int & numberLevel)
{
	///////////// инициализация ///////////////////////////
	...
	while (window.isOpen())
	{
		...
		if (numberLevel == 1 && Keyboard::isKeyPressed(Keyboard::T)) { window.close(); numberLevel++; return RunGame(numberLevel); }
		if (Keyboard::isKeyPressed(Keyboard::R)) { window.close(); return RunGame(numberLevel); }//если эскейп, то выходим из игры
